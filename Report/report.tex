...
1. We divide the predictor space - that is, the set of possible values for $X_1$, $X_2$, ..., $X_n$ for $n \in N$ into $J$ distinct and non-overlapping regions, $R_1$, $R_2$, ..., $R_J$, for $J \leq N$.

2. For every observation that falls into the region $R_j$, we make the same prediction, which is simply the \emph{most commonly occurring class, $k \in K$,} of training observations in the region to which it belongs.
...
It is a function of $\alpha$, and if you choose $\alpha=0$, then you get a tree which predicts correct for every sample. So if you where to make a lone decision tree for predicting the data, this would be the paramter that you would tweak.
...
$$
Since 0 ? p?mk ? 1, it follows that 0 ? ?p?mk log ? pmk. One can show that
the entropy will take on a value near zero if the p?mk?s are all near
